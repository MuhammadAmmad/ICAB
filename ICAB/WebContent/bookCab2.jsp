...
<%
                        ...
                        System.out.println(txtSource);
                        String txtDestination = request.getParameter("txtDestination");
                        System.out.println(txtDestination);
                        String date = request.getParameter("date");
                        String time = request.getParameter("time");
                        String city = request.getParameter("city");
                        String nseats = request.getParameter("nseats");
                        if (request.getParameter("txtDestination") != null) {

                            session.setAttribute("txtSource", txtSource);
                            System.out.println("session: "
                                    + session.getAttribute("txtSource"));
                            session.setAttribute("txtDestination", txtDestination);
                            session.setAttribute("date", date);
                            session.setAttribute("time", time);
                            session.setAttribute("city", city);
                            session.setAttribute("nseats", nseats);
                        }
                    %>
function validateForm()
{
var a=document.forms["Form"]["cabid"].value;
console.log(a);
if (a==null)
  {
  ...
  return false;
    ...
    <div class="col-lg-10 col-lg-offset-1">
            <form method="post" name="Form" onsubmit="validateForm()" action="bookCab3.jsp">
  ...
  <tbody>
                    <%
                                String sql = "select * from cab where nseats = ? and status = 'AVAILABLE'";
                                String N = (String) session.getAttribute("nseats");
                                ResultSet rs = AppDao.FetchAllDrecordsID(sql, nseats);
                                while (rs.next()) {
                            %>
                    <tr>
                        <td><input type="radio" value="<%=rs.getString(1)%>"
                                    name="cabid" required ><input type="hidden" name="nseats"
                                        value=<%=nseats%> /></td>
                ...
                <div class="form-group">
                        <button type="submit" class="btn btn-lg btn-warning">Choose Cab</button> 
                    </div><br />
                <a class='btn btn-md btn-default' href='bookCab1.jsp'>Back to Journey Details</a>
            </form>
            </div>
